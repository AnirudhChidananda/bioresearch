...
Minimum 3 tasks with agent graders (easy → medium → hard, scores between 0.01–0.99)
...
Each task defines a concrete objective an agent must accomplish, with a programmatic grader that scores performance (0.01–0.99). Tasks should range: easy → medium → hard. Graders must have clear, deterministic success/failure criteria.
...
•  Graders produce scores between 0.01–0.99?
...
No baseline inference script (inferenc.py)
